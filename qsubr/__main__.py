...
def run_qsubr(args=None):
    ...
    parser.add_argument('--cluster', '-cs', type=str, default='gizmo', help='cluster; aka -M on slurm')
    ...
    qsubr_job = qsubr.make_script(command = args.command, partition=args.partition, cluster=args.cluster, environment = args.environment, threads = args.threads, nodes = args.nodes, mem = args.mem, name = args.name, debug=args.debug, log=args.log, user=args.account)
    qsubr_job.run_job()
